...
REGISTRY: list[PortalSpec] = [
    ...
    PortalSpec("comparis.ch", Tier.T3, WAF.DATADOME, countries=["CH"], notes="DataDome [re-verified live 2026-06-09]: x-datadome:protected + set-cookie datadome + captcha body, 403 to curl_cffi Chrome. The 2026-06-04 'SSR no WAF / T1 / 214k' note was WRONG — the DOSSIER had it right (DataDome ~152k, meta-aggregator). Needs residential/mobile IP or a DataDome solver, like the other DataDome giants."),
    PortalSpec("autohero.com", Tier.T0, WAF.NONE, countries=["DE","IT","FR","ES","AT","PL","NL","SE"], notes="T2->T0 bypass: GraphQL API /v1/retail-customer-gateway/graphql/ no auth [VERIFIED 2026-06-04; re-verified live 2026-06-09 STILL VALID: POST searchAdV9AdsV2 -> 200, DE total=7155]"),
    PortalSpec("heycar.com", Tier.T0, WAF.NONE, countries=["FR"], notes="T2->T0 bypass: REST API api.fr.prod.group-mobility-trader.com/i15/search no auth, DE dead [VERIFIED 2026-06-04; re-verified live 2026-06-09 STILL VALID: GET -> 200, FR total=34239]"),
