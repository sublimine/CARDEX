"""
AutoScout24 France — EXHAUSTIVE scraper.
Same __NEXT_DATA__ structure. Decomposed by make × year.
France's malus CO2 system means imports are important — NLC computed in pipeline.
"""
...
_PAGE_SIZE = 20
_SEARCH_URL = (
    "https://www.autoscout24.fr/lst/{make_slug}"
...
class AutoScout24FR(BaseScraper):
    platform = "autoscout24_fr"
    country = "FR"
    ...
    async def crawl_shard(self, make: str, year: int) -> AsyncGenerator[RawListing, None]:
        ...
        while True:
            ...
            try:
                resp = await self.http.get(url, headers={"Accept-Language": "fr-FR,fr;q=0.9"})
                html = resp.text
            except Exception as e:
                self.log.warning("autoscout24_fr.fetch_error", url=url, make=make, year=year, error=str(e))
                break
            ...
            for item in articles:
                ...
                if not url_item.startswith("http"):
                    url_item = f"https://www.autoscout24.fr{url_item}"
                ...
                yield RawListing(
                    ...
                    region=(item.get("location") or {}).get("region"), country="FR",
