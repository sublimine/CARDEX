"""
AutoScout24 Switzerland — EXHAUSTIVE scraper.
Switzerland uses CHF → pipeline converts to EUR via FX buffer.
Same __NEXT_DATA__ structure. Decomposed by make × year.
"""
...
_PAGE_SIZE = 20
_SEARCH_URL = (
    "https://www.autoscout24.ch/lst/{make_slug}"
...
class AutoScout24CH(BaseScraper):
    ...
    async def crawl_shard(self, make: str, year: int) -> AsyncGenerator[RawListing, None]:
        ...
        while True:
            ...
            try:
                resp = await self.http.get(url, headers={"Accept-Language": "de-CH,de;q=0.9,fr-CH;q=0.8"})
                html = resp.text
            except Exception as e:
                self.log.warning("autoscout24_ch.fetch_error", url=url, make=make, year=year, error=str(e))
                break
            ...
            for item in articles:
                ...
                if not url_item.startswith("http"):
                    url_item = f"https://www.autoscout24.ch{url_item}"
                ...
                # Detect CHF vs EUR
                currency = "CHF"
                if price_info.get("currency") == "EUR":
                    currency = "EUR"
                ...
                yield RawListing(
                    ...
                    price_raw=float(raw_price) if raw_price else None, currency_raw=currency,
                    city=(item.get("location") or {}).get("city"),
                    region=(item.get("location") or {}).get("region"), country="CH",
