...
# =========================================================================== #
# 2dehands.be — migrado al sitemap de listings (multi-strategy 2026-06)
# =========================================================================== #
import json as _json  # shared by the data-route portal sections below

from scrapers.portals.sitemap_listing_base import SitemapListingScraper


@pytest.mark.unit
def test_tweedehands_is_sitemap_based() -> None:
    s = TweedehandsBEScraper()
    assert isinstance(s, SitemapListingScraper)
    assert s.SITEMAP_URL == "https://www.2dehands.be/sitemap/sitemap.xml"
    # organic car shards walked; paid admarkt mirror skipped
    assert s.CHILD_RE.search("/sitemap/l2.auto-s.volkswagen.1.sitemap.xml.gz")
    assert not s.CHILD_RE.search("/sitemap/admarkt_l2.auto-s.volkswagen.1.sitemap.xml.gz")
    assert s.DETAIL_RE.search("/v/auto-s/volkswagen/m2407378919-vw-golf")
    s._validate()
