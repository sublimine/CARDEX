...
Los valores son porcentajes de overlap esperados (basados en análisis estructural de cada familia). "—" indica ortogonalidad relativa sin overlap significativo. Los valores reales se medirán empíricamente y la matriz se actualizará.
...
Donde:
- `base_weight(family)` es el peso intrínseco por familia, calibrado:
  - A (registros legales): 0.35 (máxima autoridad)
  - B (geo): 0.15
  - C (web cartography): 0.10
  ...
  - H (OEM): 0.25
  - I (inspección): 0.05
  - J (sub-jurisdicciones): 0.10
  - K (buscadores alt): 0.05
  - L (social): 0.10
...
- `recency_factor` decae exponencialmente: reciente (1.0), hace 1 mes (0.9), 3 meses (0.7), 6 meses (0.5), >12 meses (0.3).

- `consistency_multiplier` es producto de:
