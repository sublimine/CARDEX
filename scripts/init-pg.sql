...
-- =============================================================================
-- vehicle_events: unbounded append-only log → RANGE-by-month partitions (P1.3b).
-- PK carries the partition key (event_id, ts). autovacuum stays ON (it drives
-- ANALYZE; disabling it blinds the planner — n_live_tup frozen at 0). Monthly
-- partitions are pre-created at runtime by indexer.ensure_event_partitions; the
-- DEFAULT partition guarantees an insert never fails for want of a partition.
